...
def obtener_mencion(user_id, nombre, username):
    if username:
        return f"@{username} — ID: {user_id}"
    else:
        return f"[{nombre}](tg://user?id={user_id}) — ID: {user_id}"
...
def mostrar_top(message):
    ...
    for i, (uid, count) in enumerate(top[:10], start=1):
        ...
        texto += f"{i}. {obtener_mencion(uid, info['nombre'], info.get('username'))} — {count} invitado(s)\n"
...
def mostrar_lista(message):
    ...
    for uid, info in datos.items():
        texto += f"• {obtener_mencion(uid, info['nombre'], info.get('username'))}\n"
